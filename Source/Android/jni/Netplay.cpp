...
void AndroidNetPlayUI::BootGame(const std::string& filename, std::unique_ptr<BootSessionData> boot_session_data) {
...
bool AndroidNetPlayUI::IsHosting() const { return false; }
void AndroidNetPlayUI::Update() {}
void AndroidNetPlayUI::AppendChat(const std::string& msg) {}
void AndroidNetPlayUI::OnMsgChangeGame(const NetPlay::SyncIdentifier& sync_identifier, const std::string& netplay_name) {
...
bool AndroidNetPlayUI::IsRecording() { return false; }

std::string AndroidNetPlayUI::FindGBARomPath(const std::array<u8, 20>& hash, std::string_view title, int device_number) { return ""; }
...
void AndroidNetPlayUI::ShowChunkedProgressDialog(const std::string& title, u64 data_size, const std::vector<int>& players) {}
...
std::shared_ptr<const UICommon::GameFile> AndroidNetPlayUI::FindGameFile(const NetPlay::SyncIdentifier& sync_identifier,
